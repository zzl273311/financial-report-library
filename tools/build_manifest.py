...
SOURCES = [
    ...
    dict(src='投资组合筛选/投资组合筛选_10家公司排序.html', slug='portfolio-screen',
         company='投资组合筛选', code='55 家 → 10 家', sector='跨公司比较', short='组合',
         period='2026-09 第 3 版', note='基于 54 份报告（覆盖 55 家公司）的质量—安全—回报排序',
         tags=['五维评分', '样本扩充至 55 家', '组合结构', '拒绝采信清单', '五类高质量假象'],
         manual=dict(summary='从工作区 54 份已生成的财报分析报告（覆盖 55 家公司）中，按「生意质量 25% ＋ 盈利质量与现金转化 20% ＋ 成长性 20% ＋ 资产负债表安全 20% ＋ 股东回报 15%」五维加权评分选出 10 家并排序：贵州茅台、海天味业、中国海油、农夫山泉、腾讯控股、药明康德、紫金矿业、洛阳钼业、网易、渣打银行。第 3 版新增 11 家公司（紫金矿业、洛阳钼业、渣打银行、中银香港、汇丰控股、珀莱雅、香港中华煤气、浦发银行、上海银行、北京银行、美团），其中前三家评分高于第 2 版第 10 名，直接挤掉山西汾酒、中国平安与招商银行。报告明确声明：54 家中 53 家无 PE / PB / 市值（仅携程 1 家有），因此本排序是质量驱动的排序、不是性价比排序；同时设"拒绝采信清单"（五粮液异常数据、东阿阿胶不可引用列、华润医药税率型增长），系统拆解"五类高质量假象"（非现金利润、资产处置型增长、以信用换收入、加杠杆分红、行业利润与现金流长期背离），并逐家说明落选原因与评分—排序差异，含"资源合并敞口 28% 越线"的主动坦白。',
                     metrics=[])),
]
