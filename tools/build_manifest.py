...
SOURCES = [
    ...
    dict(src='投资组合筛选/投资组合筛选_10家公司排序.html', slug='portfolio-screen',
         ...
         period='2026-09 第 2 版', note='基于 43 份报告（覆盖 44 家公司）的质量—安全—回报排序',
         tags=['五维评分', '样本扩充至 44 家', '组合结构', '拒绝采信清单'],
         manual=dict(summary='从工作区 43 份已生成的财报分析报告（覆盖 44 家公司）中，按「生意质量 25% ＋ 盈利质量与现金转化 20% ＋ 成长性 20% ＋ 资产负债表安全 20% ＋ 股东回报 15%」五维加权评分选出 10 家并排序：贵州茅台、腾讯控股、中国海油、海天味业、农夫山泉、药明康德、网易、山西汾酒、中国平安、招商银行。报告明确声明：43 家中 42 家无 PE / PB / 市值（仅携程 1 家有），因此本排序是质量驱动的排序、不是性价比排序；同时设"拒绝采信清单"（五粮液异常数据、东阿阿胶不可引用列、华润医药税率型增长），并逐家说明落选原因与评分—排序差异。',
                     metrics=[])),
]
